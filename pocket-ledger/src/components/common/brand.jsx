...
import { bankFor, networkFor, subBrandFor, logoUrl } from "../../lib/brands.js";

/* Real logo image with graceful fallback: the drawn mark shows immediately
   and is swapped only once the online logo has actually loaded — never an
   empty box (slow network) or a broken image (offline). Google's generic
   16px globe placeholder is rejected so unknown domains keep the fallback. */
function BrandImg({ domain, size, fallback }) {
  const [state, setState] = useState("loading");
  if (!domain || state === "err") return fallback;
  return (
    <>
      {state !== "ok" && fallback}
      <img
        src={logoUrl(domain)} width={size} height={size} alt="" loading="lazy"
        style={{ objectFit: "contain", display: state === "ok" ? "block" : "none" }}
        onLoad={(e) => setState(e.target.naturalWidth > 16 ? "ok" : "err")}
  ...
  return (
    ...
    >
      <span className="absolute" style={{ top: Math.round(h * 0.12), insetInlineStart: Math.round(width * 0.09) }}>
        {bank
          ? <BrandImg domain={bank.domain} size={Math.round(h * 0.5)} fallback={<span className="ui" style={{ fontWeight: 800, fontSize: Math.max(6, width * 0.16), color: bank.color, lineHeight: 1 }}><span style={{ color: bank.accent }}>◆</span>{bank.label}</span>} />
...
export function SubLogo({ name, size = 38, tintBg, tintColor }) {
  ...
  const glyph = (
    <span
      className={`flex items-center justify-center w-full h-full ${b.serif ? "disp" : "ui"}`}
      style={{ background: b.bg, color: b.fg, fontWeight: 800, fontSize: b.small ? size * 0.24 : size * 0.45 }}
    >
      {b.ch}
    </span>
  );
