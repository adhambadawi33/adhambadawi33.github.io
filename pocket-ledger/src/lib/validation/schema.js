...
export function normalizeRecurring(r, report, accountIds) {
  ...
  const out = {
    ...
    /* "Needs cancelling" watchlist: keep to-stop subs in sight
...
function normalizeSettings(s) {
  ...
  return {
    ...
    lock: s.lock && typeof s.lock === "object" && str(s.lock.pinHash) ? { pinHash: str(s.lock.pinHash), ...(str(s.lock.credId) ? { credId: str(s.lock.credId) } : {}) } : null,
